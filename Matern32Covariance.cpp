...
// [[Rcpp::export]]
NumericMatrix Matern32Covariance(NumericMatrix X1, NumericMatrix X2, double l, double sigma2, int trainingC) {
  // C = GaussianCovariance(x1,x2,l,sigma2,training)
  // 
  ...
  if (trainingC == 1){
    ...
    for(int ii = 0; ii < C.nrow(); ii++){
      for(int jj = ii+1; jj < C.ncol(); jj++){
        d = sqrt(3*sum(pow(X1.row(ii)-X2.row(jj),2))/pow(l,2));
        C(ii,jj) = sigma2*(1+d)*exp(-d);
        C(jj,ii) = sigma2*(1+d)*exp(-d);
      }
      C(ii,ii) = sigma2;
    }
  }
